...
function questions_answers_main()
{
    ...
    if (isset($_REQUEST['action']))
    {
        ...
        if ($_REQUEST['action']=="question_send")  question_send();   
    }
...
function questions_answers_list()       // список всех вопросов
{
	...
	if ($user_id==0) $add_link="<table border=0><tr><td><a href='/auth/register/'><img width=24px src='/img/add.png' border=0></a></td><td><a href='/auth/register/'>Добавить вопрос!</a></td></tr></table>"; else $add_link="<div style='padding:50 0 20 0;'><a href='/".$path_questions."?action=add_question'>Добавить вопрос...</a></div>";
	$out=$add_link."<table cellpadding=2 cellspacing=2 border=0>
						<tr>
                            <td width=5%> </td>
                            <td width=15%> </td>
                            <td width=30%> </td>
...
{
	global $user_id, $sql_pref, $conn_id, $path, $art_url, $path_questions, $path_users, $path_companies;
	global $page_title, $page_header1;
	global $module_name, $module_url;
	$out="";
	
 	$sql_query="SELECT id, dt, question, user_id, question_type, rate FROM ".$sql_pref."_questions WHERE id='".$art_url."' AND enable='Yes'";
	$sql_res=mysql_query($sql_query, $conn_id);
	if (mysql_num_rows($sql_res)>0)
	{
		list($id, $dt, $question, $p_user_id, $question_type, $rate)=mysql_fetch_row($sql_res);
	    //echo $question_type;
        ...
        $vote_up.="<img style='cursor: pointer;' onclick='javascript:vote_up(this,event)' src='/img/up.gif' border=1 alt='Вопрос нравится'>";
        $vote_down.="<img style='cursor: pointer;' onclick='javascript:vote_down(this,event)' src='/img/down.gif' border=1 alt='Вопрос не нравится'>";
        
        if(action_check('quest_vote',$id)=='true'||$user_id==$p_user_id||$user_id==0)
        {$question_show="<br><input type='hidden' id='question_id' value='".$id."'></input>Оценка:<span style='border: 1; background-color: #AEE2EC; width: 30; height: 20; text-align: center' id='rate'>".$rate."</span><span style='visibility: visible;' id='result'></span>";}
        else{$question_show="<br><input type='hidden' id='question_id' value='".$id."'></input>Оценка:<span style='border: 1; background-color: #AEE2EC; width: 30; height: 20; text-align: center'  id='rate'>".$rate."</span><span style='visibility: visible;' id='result'> ".$vote_up." ".$vote_down."</span>";}
        
        if (!empty($question)) $question_show.="<div style=' background-color: #BDEBB1;'>".$question."</div>"; else $question_show.="<div style='padding: 3 0;'>".$question."</div>";
        ...
        $p_user_show="<div style='padding: 3 0; '>".$p_user_name."</div>";
		...
		$out.=" <table cellpadding=0 cellspacing=0 border=0 width=100%>
					<tr>
						<td valign=top>".$dt_show.$question_show."</td>
					</tr>
				</table>";
...
function questions_line()       // опрос на главную страницу
{
	...
	global $module_name, $module_url;
	$out="";
 	$sql_query="SELECT id, dt, question, user_id, question_type FROM ".$sql_pref."_questions WHERE (enable='Yes' AND main='Yes' AND question_type=2) ORDER BY dt DESC LIMIT 1 ";
	$sql_res=mysql_query($sql_query, $conn_id);
	if (mysql_num_rows($sql_res)>0)
	{        
        $out.="<table cellpadding=0 cellspacing=0 border=0 width='100%' background='/img_new/int/main-bg.gif'>
                <tr>
                    <td valign=top background='/img_new/int/main-top.gif' style='background-image: /img_new/int/main-top.gif; background-position: top; background-repeat: repeat-x;'>
                        ...
                        <h2 style='padding: 0 0 10 0;'>Опрос энергетиков</h2>
                        ";
        ...
        $out.="<div>".$comments_show."<br/><br/><a href='/".$path_questions."/'>Все опросы</a></div>";
        $out.="                
                        </div>
                    </td>
                </tr>
...
function questions_answers_add_form()           // форма добавления вопроса
{
	...
	$out.="<form name='form_name' method='post'>
            ...
            <table cellpadding=0 cellspacing=0 border=0 width=100%>	
            	<tr>
           	    ...
           	    <tr>
            		...
            		<td class='form_main'><input type='radio' onclick='display_div(10)' name='question_type' checked='yes' value='1'> - вопрос &nbsp; &nbsp; &nbsp; <input type='radio' onclick='display_div(1)' name='question_type' value='2'> - опрос</td>
            	</tr>       	    
                ...
                <tr style='display: none;' id='div_2'>
        		   <td>Вариант 1</td>
        		   <td><textarea name='answer1' id='answer_1' rows='2' style='overflow: auto; font-size: 12px; width: 500px;'></textarea><input type='button' name='button_1' onclick='display_div(3);' value='...'></td>
        	    </tr>
            ...
            <input class='form_button' type='submit' name='button_submit' value='Добавить вопрос' style='width: auto;'>
           </form>";
	return ($out);    
}
...
function add_question_to_base()     // добавление вопроса в БД
{
    ...
    $sql_query2="INSERT INTO ".$sql_pref."_users_action (user_id, action_type, row_id) VALUES ('".$user_id."', 'add_question','".$question_id."')";
    $sql_res2=mysql_query($sql_query2, $conn_id); 
    ...
    header("location:/auth/questions_list");
    //echo $sql_query;	
}
...
function questions_answer_form_save()           // сохранение ответа
{
	...
	if (isset($_REQUEST['content1']) && !empty($_REQUEST['content1']))
	{
        ...
        $sql_query2="INSERT INTO ".$sql_pref."_users_action (user_id, action_type, row_id) VALUES ('".$user_id."', 'question_comment','".$parent_id."')";
        $sql_res2=mysql_query($sql_query2, $conn_id);
        //echo $sql_query."___".$answer_id;
        if($answer_id>0)
        {
...
function question_send()                 // рассылка вопроса зарегистрированным пользователям
{
	global $sql_pref, $conn_id, $path, $page_header1, $path_questions, $user_id, $user_status, $art_url;
	$out="";
	
	if (!isset($_REQUEST['question_id']) || ($_REQUEST['question_id']<=0)) return; else $question_id=$_REQUEST['question_id'];
    if($question_id>0 && $user_id>0)
    {
   	    $sql_query0="UPDATE ".$sql_pref."_questions SET send='Yes' WHERE id='".$question_id."'";
        $sql_res0=mysql_query($sql_query0, $conn_id);
        
        $sql_query="SELECT ".$sql_pref."_questions.id, ".$sql_pref."_users.id, ".$sql_pref."_users.surname, ".$sql_pref."_users.name, ".$sql_pref."_users.name2, ".$sql_pref."_users.email
                     FROM ".$sql_pref."_questions INNER JOIN ".$sql_pref."_users ON ".$sql_pref."_questions.user_id=".$sql_pref."_users.id  WHERE ".$sql_pref."_questions.id=".$question_id;
        $sql_res=mysql_query($sql_query, $conn_id);
        if (mysql_num_rows($sql_res)>0)
        {
            list($quest_id, $id_from, $surname_from, $name_from, $name2_from, $email_from)=mysql_fetch_row($sql_res);
            ...
            $sql_query2="SELECT id FROM ".$sql_pref."_users WHERE enable='Yes' AND maillist='Yes' AND actkod='' AND id<>".$user_id;
	        $sql_res2=mysql_query($sql_query2, $conn_id);            
        	if (mysql_num_rows($sql_res2)>0)
        	{ 
        	    while(list($id)=mysql_fetch_row($sql_res2))
                {      
                    $letter_name="www.ensor.ru: Приглашение к обсуждению вопроса, опубликованного пользователем ".$name_from.".";
                    $letter_content_send="Пользователь ".$name_from." опубликовал(а) вопрос в рубрике вопросы/ответы.\n";                            
                    $letter_content_send.="Чтобы присоединиться к обсуждению - перейдите по ссылке: http://www.ensor.ru/discussions/questions_answers/".$question_id.".html.\n";
                    $letter_content_send.="Если указанная выше ссылка не открывается, скопируйте ее в буфер обмена, вставьте в адресную строку браузера и нажмите ввод.\n\n";
                    $letter_content_send.="Вы получили это письмо, потому что зарегистрированы на сайте www.ensor.ru.\n\n";
                    $letter_content_send.="--\nС уважением,\nСлужба поддержки www.ensor.ru.\n";
                    $letter_content_send.="--------------------------------------------------------------";
