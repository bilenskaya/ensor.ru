...
function demand_main()
{
	global $sql_pref, $conn_id, $art_url;
	$out="";
    
	if (isset($art_url)) $out.=demand_out();
	else $out.=demand_list();
...
{
	global $sql_pref, $conn_id, $path,$path_contacts, $path_demand, $path_companies, $demand_perpage, $user_id;
	$out="";
	
	if (isset($_REQUEST['page'])&& $_REQUEST['page']>0) $page=$_REQUEST['page']; else $page=1;
	$perpage=$demand_perpage; $first=$perpage*($page-1);
	$sql_query="SELECT id FROM ".$sql_pref."_demand WHERE enable='Yes'";
	$pages_show="<div align=left style='padding: 20 0 10 0;'>".pages_nums($page, $perpage, $sql_query)."</div>";

	$sql_query="SELECT ".$sql_pref."_demand.id, ".$sql_pref."_demand.dt, ".$sql_pref."_demand.descr, ".$sql_pref."_demand.content, ".$sql_pref."_demand.user_id, ".$sql_pref."_demand.company_id, ".$sql_pref."_users.rate_sec, ".$sql_pref."_demand.top_time FROM ".$sql_pref."_demand INNER JOIN ".$sql_pref."_users ON ".$sql_pref."_demand.user_id=".$sql_pref."_users.id WHERE ".$sql_pref."_demand.enable='Yes' AND ".$sql_pref."_users.enable='Yes' ORDER BY ".$sql_pref."_demand.top_time DESC, ".$sql_pref."_demand.dt DESC LIMIT ".$first.",".$perpage;
...
function demand_out()
{
	global $sql_pref, $conn_id, $path, $art_url, $path_demand, $path_users, $path_companies;
	global $page_title, $page_header1;
	global $module_name, $module_url;
	$out="";
	
 	$sql_query="SELECT id, dt, descr, content, tags, sfera_ids, direction_ids, user_id, company_id FROM ".$sql_pref."_demand WHERE id='".$art_url."' AND enable='Yes'";
	$sql_res=mysql_query($sql_query, $conn_id);
	if (mysql_num_rows($sql_res)>0)
	{
        ...
        //$out.="<div style='padding: 30 0 10 0;'>".demand_feedback($id)."</div>";
        
		$out.="<div style='padding:50 0 20 0;'><a href='/".$path_demand."/'>К списку поиска исполнителей...</a></div>";
