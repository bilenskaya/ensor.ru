...
function insert_cut(tagName){
	var startTag = '<blogocut=\"„итать далее...\">';
	var endTag = '';
	insert_tag(startTag, endTag);	
	return false;
}
...
function insert_image(){
	var src = prompt('”кажите адрес изображени€', 'http://');
	if(src){
		insert_tag('<img src="' + src + '" alt="»зображение"/>', '');
	}
}

function insert_link(){
	var href = prompt('”кажите адрес ссылки', 'http://');
	if(href){
