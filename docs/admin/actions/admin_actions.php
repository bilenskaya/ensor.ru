...
function actions_show()
{
	...
	$sql_query="SELECT id, user_id,  date, action_type, row_id FROM ".$sql_pref."_users_action ORDER BY id DESC".$pref_page;
	$sql_res=mysql_query($sql_query, $conn_id);
	if (mysql_num_rows($sql_res)>0)
	{
		while (list($id, $user_id, $dt, $action_type, $row_id)=mysql_fetch_row($sql_res))
		{
			$action_type=stripslashes($action_type);
			$dt_show=$dt;
			if ($dt_show=="0000-00-00") $dt_show=" - ";
			if (empty($action_type)) $action_type="<i><без заголовка></i>";
//			if ($enable=='Yes') $enable_pic="<a href='?id=".$id."&action=actions_enable'><img src='/admin/img/check_yes.gif' width=25 height=13 alt='Отображение раздела' border=0></a>"; else $enable_pic="<a href='?id=".$id."&action=actions_enable'><img src='/admin/img/check_no.gif' width=25 height=13 alt='Отображение раздела' border=0></a>";
//			$edit_pic="<a href='?id=".$id."&action=actions_edit#actions_edit'><img src='/admin/img/edit.gif' width=25 height=13 alt='Редактировать' border=0></a>";
			$del="<a href=\"javascript:if(confirm('Вы уверены?'))window.location='?id=".$id."&action=actions_delete'\"><img src='/admin/img/del.gif' width=25 height=13 alt='Удалить' border=0></a>";
			echo "<tr class='common'>
					<td class='common' align='center'><font color='#A0A0A0'>".$id."</font></td>
					<td class='common' align='center'>".$dt_show."</td>
					<td class='common' align='center'>".$edit_pic."</td>
					<td class='common' align='left'>Пользователь [".get_user_name_by_id($user_id)."].   Действие [".$action_type."].    Строка[".$row_id."]</td>
					<td class='common' align='center'>".$del."</td>
				</tr>";
		}
	}
	echo "</table>";
	$sql_query="SELECT id FROM ".$sql_pref."_users_action";
	$sql_res=mysql_query($sql_query, $conn_id);
...
function form_actions_save()
{
	...
	if (is_uploaded_file( $_FILES['file_name']['tmp_name'])) form_actions_mainimg_save($pic_id);
