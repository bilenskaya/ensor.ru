...
function articles_show()
{
	...
	echo "<table class='main' cellspacing='2' cellpadding='2' width='100%'>
			<tr class='maintitle'>
				...
				<td width='200' class='maintitle' align='center'><b>url</b></td>
				<td width='30' class='maintitle' align='center'><b>del</b></td>
			</tr>";

	$sql_query="SELECT id, url, enable, name FROM ".$sql_pref."_articles ORDER BY name";
	$sql_res=mysql_query($sql_query, $conn_id);
	if (mysql_num_rows($sql_res)>0)
	{
		while (list($id, $url, $enable, $name)=mysql_fetch_row($sql_res))
		{
			$name=stripslashes($name);$url=stripslashes($url);
			if (empty($name)) $name="<i><без заголовка></i>";
			if ($enable=='Yes') $enable_pic="<a href='?id=".$id."&action=articles_enable'><img src='/admin/img/check_yes.gif' width=25 height=13 alt='Отображение' border=0></a>"; else $enable_pic="<a href='?id=".$id."&action=articles_enable'><img src='/admin/img/check_no.gif' width=25 height=13 alt='Отображение' border=0></a>";
			if (is_dir($path."files/articles/".$id) && is_cat_empty($path."files/articles/".$id)==false)  $imgs="<a href='?id=".$id."&action=articles_images#articles_images'><img src='/admin/img/img.gif' width=25 height=13 alt='Дополнительные изображения' border=0></a>"; else $imgs="<a href='?id=".$id."&action=articles_images#articles_images'><img src='/admin/img/img_ina.gif' width=25 height=13 alt='Дополнительные изображения' border=0></a>";
			$edit_pic="<a href='?id=".$id."&action=articles_edit#articles_edit'><img src='/admin/img/edit.gif' width=25 height=13 alt='Редактировать' border=0></a>";
			$del="<a href=\"javascript:if(confirm('Вы уверены?'))window.location='?id=".$id."&action=articles_delete'\"><img src='/admin/img/del.gif' width=25 height=13 alt='Удалить' border=0></a>";
	...
	echo "</table>";
	echo "<br><li><a href='?action=articles_add#articles_add'>Добавить</a></li>";
	echo "<hr>";
}
...
function form_articles_save()
{
	...
	while ($fl==1)
	{
		...
		$sql_query="SELECT url FROM ".$sql_pref."_articles WHERE url='".$url."'".$pref_id."";
		$sql_res=mysql_query($sql_query, $conn_id);
	...
	{
		$sql_query="UPDATE ".$sql_pref."_articles SET url='".$url."', enable='".$enable."', dt='".$dt."', name='".$name."', descr='".$descr."', content='".$content."', tags='".$tags."', category='".$category."' WHERE id='".$_REQUEST['id']."'";
		$sql_res=mysql_query($sql_query, $conn_id);
		$pic_id=$_REQUEST['id'];
	}
	else
	{
		$sql_query="INSERT INTO ".$sql_pref."_articles (url, enable, dt, name, descr, content, tags, category) VALUES ('".$url."', '".$enable."', '".$dt."', '".$name."', '".$descr."', '".$content."', '".$tags."', '".$category."')";
		$sql_res=mysql_query($sql_query, $conn_id);
		$pic_id=mysql_insert_id();
	}
	if (is_uploaded_file( $_FILES['file_name']['tmp_name'])) form_articles_mainimg_save($pic_id);
...
function form_articles_mainimg_save($pic_id)
{
	global $path;
	global $articles_img_width, $articles_img_height, $articles_img_thumb_width, $articles_img_thumb_height;
	...
	if ($mime=="image/jpeg" || $mime=="image/pjpeg" || $mime=="image/gif")
	{
		...
		$dest=$path."files/articles/imgs/".$pic_id.$ext;
		$dest1=$path."files/articles/imgs/".$pic_id.$ext1;
		$src=$_FILES["file_name"]["tmp_name"];
		$resize=true;
		del_file($dest);del_file($dest1);
		save_img($src, $dest, $resize, $articles_img_width, $articles_img_height, 80, $mime);

		$thumb_src=$dest;
		$thumb_dest=$path."files/articles/thumbs/".$pic_id.$ext;
		$thumb_dest1=$path."files/articles/thumbs/".$pic_id.$ext1;
		$thumb_resize=true;
		del_file($thumb_dest);del_file($thumb_dest1);
		save_img($thumb_src, $thumb_dest, $thumb_resize, $articles_img_thumb_width, $articles_img_thumb_height, 80, $mime);
	}
...
function form_articles_images_save()
{
	global $path;
	...
	if ($mime=="image/jpeg" || $mime=="image/pjpeg" || $mime=="image/gif")
	{
		...
		$dest=$path."files/articles/".$_REQUEST['id']."/".translit_url($filename).$ext;
		$src=$_FILES["file_name"]["tmp_name"];
		...
		make_dir($path."files/articles/".$_REQUEST['id']);
		del_file($dest);
