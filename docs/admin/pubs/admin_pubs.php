...
function rubs_show()
{
	...
	subrubs_show(0);
	echo "</table><br>";
	echo "<div style='padding:20 0 20 0;'><a href='?action=styles_edit#styles_edit'>styles.css</a></div>";
}
...
function subrubs_show($par_id)
{
	...
	$sql_query="SELECT id, url, enable, name, content, parent_id, code, level, module FROM ".$sql_pref."_pub_rubs WHERE parent_id=".$par_id." ORDER BY code";
	...
	if (mysql_num_rows($sql_res)>0)
	{
		while (list($id, $url, $enable, $name, $content, $parent_id, $code, $level, $module)=mysql_fetch_row($sql_res))
		{
			...
			else
			{
				...
				$sql_query_1="SELECT id FROM ".$sql_pref."_pub_rubs WHERE parent_id='".$parent_id."' AND code='".($code+1)."'"; $sql_res_1=mysql_query($sql_query_1, $conn_id); 
				...
				$sql_query_1="SELECT id FROM ".$sql_pref."_pub_arts WHERE parent_id='".$id."'";
				$sql_res_1=mysql_query($sql_query_1, $conn_id);
				$art_kol=mysql_num_rows($sql_res_1);
				$sql_query_1="SELECT id FROM ".$sql_pref."_pub_rubs WHERE parent_id='".$id."'";
				$sql_res_1=mysql_query($sql_query_1, $conn_id);
				...
				while ($parid[($i-1)]!=0)
				{
					$sql_query1="SELECT parent_id FROM ".$sql_pref."_pub_rubs WHERE id=".$parid[($i-1)];
...
function form_rub_save()
{
	...
	while ($fl==1)
	{
		...
		$sql_query="SELECT url FROM ".$sql_pref."_pub_rubs WHERE parent_id='".$parent_id."' AND url='".$url."'".$pref_id."";
		$sql_res=mysql_query($sql_query, $conn_id);
	...
	{
		$sql_query="UPDATE ".$sql_pref."_pub_rubs SET url='".$url."', enable='".$enable."', name='".$name."', content='".$content."', auth='".$auth."', module='".$module."', adv_content='".$adv_content."' WHERE id='".$_REQUEST['id']."'";
		$sql_res=mysql_query($sql_query, $conn_id);
	}
	else
	{
		$sql_query="SELECT id FROM ".$sql_pref."_pub_rubs WHERE parent_id='".$parent_id."'";
		$sql_res=mysql_query($sql_query, $conn_id);
		$code=mysql_num_rows($sql_res)+1;
		$level=$_REQUEST['level']+1;
		$sql_query="INSERT INTO ".$sql_pref."_pub_rubs (enable, url, name, content, parent_id, code, level, auth, module) VALUES ('".$enable."', '".$url."', '".$name."', '".$content."', '".$parent_id."', '".$code."', '".$level."', '".$auth."', '".$module."')";
		$sql_res=mysql_query($sql_query, $conn_id);
	}
}
...
function form_art_save()
{
	...
	{
		if (isset($_REQUEST['id']) && !empty($_REQUEST['id'])) $pref_id=" AND id<>'".$_REQUEST['id']."'"; else $pref_id="";
		$sql_query="SELECT url FROM ".$sql_pref."_pub_arts WHERE parent_id='".$parent_id."' AND url='".$url."'".$pref_id."";
		$sql_res=mysql_query($sql_query, $conn_id);
		if (mysql_num_rows($sql_res)>0) {$url.=$i;$i++;}
		else $fl=0;
	}
	...
	else
	{
		...
		$sql_res=mysql_query($sql_query, $conn_id);
		$code=mysql_num_rows($sql_res)+1;
		$sql_query="INSERT INTO ".$sql_pref."_pub_arts (enable, url, name, descr, content, parent_id, code, title) VALUES ('".$enable."', '".$url."', '".$name."', '".$descr."', '".$content."', '".$parent_id."', '".$code."', '".$title."')";
		$sql_res=mysql_query($sql_query, $conn_id);
	}
}
