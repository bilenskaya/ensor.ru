...
function check_empty($id)
{
	...
	$sql_query2="SELECT id FROM ".$sql_pref."_useful WHERE parent_id REGEXP '\"$id\"'";
...
function admin_subrubric_list ($sub_id, $sub_level)
{
	global $sql_pref, $conn_id;
	$sub_level++;
	$sql_query="SELECT id, enable, name FROM ".$sql_pref."_useful_rub WHERE parent_id='$sub_id' ORDER BY name";
...
function admin_show_useful($rub_id)
{
	global $sql_pref, $conn_id, $path;
	$add_pic="<a href='?id=".$rub_id."&action=useful_add#useful_add'><img src='/admin/img/line.gif' width=25 height=13 alt='Добавить каталог' border=0></a>";
	$sql_query="SELECT id, parent_id, name, enable, date_upload, date_modify, date_issue, format, file_size, file_name, org_id, tags, user_id FROM ".$sql_pref."_useful WHERE (parent_id REGEXP '\"$rub_id\"' AND moderation = 'No')";
	$sql_res=mysql_query($sql_query, $conn_id);
	echo "<table class='main' cellspacing='2' cellpadding='2' width='100%'>
    			<tr class='maintitle'>
    				<td width='20' class='maintitle' align='center'><b>id</b></td>
	...
	if (mysql_num_rows($sql_res)>0)
	{
		while(list($id, $parent_id, $name, $enable, $date_upload, $date_modify, $date_issue, $format, $file_size, $file_name, $org_id, $tags, $user_id)=mysql_fetch_row($sql_res))
		{
			...
			if ($enable=='Yes') $enable_pic="<a href='?id=".$id."&action=useful_enable&rub_id=".$rub_id."'><img src='/admin/img/check_yes.gif' width=25 height=13 alt='Отображение' border=0></a>"; else $enable_pic="<a href='?id=".$id."&action=useful_enable&rub_id=".$rub_id."'><img src='/admin/img/check_no.gif' width=25 height=13 alt='Отображение' border=0></a>";
			
			$edit_pic="<a href='?id=".$id."&rub_id=".$rub_id."&action=useful_edit#useful_edit'><img src='/admin/img/edit.gif' width=25 height=13 alt='Редактировать' border=0></a>";
			$del="<a href=\"javascript:if(confirm('Вы уверены?'))window.location='?id=".$id."&rub_id=".$rub_id."&action=useful_delete'\"><img src='/admin/img/del.gif' width=25 height=13 alt='Удалить' border=0></a>";
			if (file_exists($path."files/useful/catalog/imgs/".$id.".jpg")) $fname=$id.".jpg";
	elseif (file_exists($path."files/useful/catalog/imgs/".$id.".gif")) $fname=$id.".gif";
	...
	if (mysql_num_rows($sql_user_res)>0) {list($surname, $user_name, $name2)=mysql_fetch_row($sql_user_res); $full_user=$surname." ".$user_name." ".$name2;}
					...
					if (isset($fname)) echo "<img src='/files/useful/catalog/imgs/".$fname."' border=1>";
						else echo "<BR>";
					unset($fname);
					echo "</td>
					<td class='common' align='left'>".$name."</td>
					<td class='common' align='left'>".$date_issue."</td>";
					if (strlen($file_name)>0 and file_exists($path."files/useful/catalog/".$file_name))
...
function admin_show_moderate()
{
	...
	$sql_query="SELECT id, parent_id, name, enable, date_upload, date_modify, date_issue, format, file_size, file_name, org_id, tags, user_id FROM ".$sql_pref."_useful WHERE moderation='Yes'";
	//echo $sql_query;
    $sql_res=mysql_query($sql_query, $conn_id);
	...
	if (mysql_num_rows($sql_res)>0)
	{
		while(list($id, $parent_id, $name, $enable, $date_upload, $date_modify, $date_issue, $format, $file_size, $file_name, $org_id, $tags, $user_id)=mysql_fetch_row($sql_res))
		{
			...
			$del="<a href=\"javascript:if(confirm('Вы уверены?'))window.location='?id=".$id."&rub_id=moderate&action=useful_delete'\"><img src='/admin/img/del.gif' width=25 height=13 alt='Удалить' border=0></a>";
			if (file_exists($path."files/useful/catalog/imgs/".$id.".jpg")) $fname=$id.".jpg";
	elseif (file_exists($path."files/useful/catalog/imgs/".$id.".gif")) $fname=$id.".gif";
	$sql_org_query="SELECT name FROM ".$sql_pref."_companies WHERE id='$org_id'";
	$sql_org_res=mysql_query($sql_org_query, $conn_id);
	...
	if (mysql_num_rows($sql_user_res)>0) {list($surname, $user_name, $name2)=mysql_fetch_row($sql_user_res); $full_user=$surname." ".$user_name." ".$name2;}
					...
					<td class='common' align='left'>".$date_issue."</td>";
					if (strlen($file_name)>0 and file_exists($path."files/useful/catalog/".$file_name))
						echo "<td class='common' align='left'><a href=/files/useful/catalog/".$file_name.">".$file_name."</a></td>";
						else echo "<td class='common' bgcolor='red' align='left'>".$file_name."</td>";
					echo "<td class='common' align='left'>".$file_size."</td>
					<td class='common' align='left'>".$format."</td>
					<td class='common' align='left'>".$org_name."</td>
...
function admin_show_rubric($rub_id)
{
	...
	$sql_query="SELECT id, name, descr FROM ".$sql_pref."_useful_rub WHERE id='$rub_id'";
	$sql_res=mysql_query($sql_query, $conn_id);
	if (mysql_num_rows($sql_res)>0)
	{
	...
	elseif (file_exists($path."files/useful/rubric/imgs/".$id.".gif")) $fname=$id.".gif";
					...
					<tr class='common'>
					<td width=10%><img src='/files/useful/rubric/imgs/".$fname."' border=1></td><td align='center'>".$descr."</td>
					</tr>
					</table>";
		admin_show_useful($id);
		}
	}
}
...
function form_rubric_save()
{
	...
	{
		$sql_query="UPDATE ".$sql_pref."_useful_rub SET name='".$name."', parent_id='".$parent_id."', descr='".$descr."', enable='".$enable."' WHERE id='".$_REQUEST['id']."'";
		$sql_res=mysql_query($sql_query, $conn_id);
		$pic_id=$_REQUEST['id'];
	}
	else
	{
		$sql_query="INSERT INTO ".$sql_pref."_useful_rub (name, parent_id, descr, enable) VALUES ('".$name."', '".$parent_id."', '".$descr."', '".$enable."')";
		$sql_res=mysql_query($sql_query, $conn_id);
...
if (is_uploaded_file( $_FILES['file_name']['tmp_name'])) 
	{
	if (isset($_REQUEST['id']) && !empty($_REQUEST['id'])) $replace_useful=$id; else $replace_useful="";
	$data_file=form_useful_file_save($replace_useful);
	$file_name=$data_file["name"];
	$file_size=$data_file["size"];
	$format=$data_file["type"];
	if (isset($_REQUEST['id']) && !empty($_REQUEST['id']))
		{
		$sql_query="UPDATE ".$sql_pref."_useful SET file_name='".$file_name."', file_size='".$file_size."', format='".$format."' WHERE id='".$_REQUEST['id']."'";
		$sql_res=mysql_query($sql_query, $conn_id);
		}
	}
	...
	if (isset($_REQUEST['id']) && !empty($_REQUEST['id']))
	{
		$sql_query="UPDATE ".$sql_pref."_useful SET parent_id='".$parent_id."', name='".$name."', date_issue='".$date_issue."', org_id='".$org_id."', user_id='".$user_id."', date_modify='".$date_modify."', tags='".$tags."', enable='".$enable."' WHERE id='".$_REQUEST['id']."'";
		$sql_res=mysql_query($sql_query, $conn_id);
		$pic_id=$_REQUEST['id'];
	}
	else
	{
		$sql_query="INSERT INTO ".$sql_pref."_useful (parent_id, name, file_name, file_size, format, date_issue, org_id, user_id, date_upload, date_modify, tags, enable) VALUES ('".$parent_id."', '".$name."', '".$file_name."', '".$file_size."', '".$format."', '".$date_issue."', '".$org_id."', '".$user_id."','".$date_upload."', '".$date_modify."', '".$tags."', '".$enable."')";
		$sql_res=mysql_query($sql_query, $conn_id);
		$pic_id=mysql_insert_id();
	}
	if (is_uploaded_file( $_FILES['img_name']['tmp_name'])) form_useful_mainimg_save($pic_id);
...
function form_rubric_mainimg_save($pic_id)
{
	...
	if ($mime=="image/jpeg" || $mime=="image/pjpeg" || $mime=="image/gif")
	{
		...
		$dest=$path."files/useful/rubric/imgs/".$pic_id.$ext;
		$dest1=$path."files/useful/rubric/imgs/".$pic_id.$ext1;
		...
		$thumb_src=$dest;
		$thumb_dest=$path."files/useful/rubric/thumbs/".$pic_id.$ext;
		$thumb_dest1=$path."files/useful/rubric/thumbs/".$pic_id.$ext1;
		$thumb_resize=true;
...
function form_useful_mainimg_save($pic_id)
{
	global $path, $equipment_useful_img_width, $equipment_useful_img_height;
	...
	if ($mime=="image/jpeg" || $mime=="image/pjpeg" || $mime=="image/gif")
	{
		...
		$resize=true;
		del_file($dest);del_file($dest1);
		save_img($src, $dest, $resize, $equipment_useful_img_width, $equipment_useful_img_height, 80, $mime);
	}
	else echo "Ошибка! Недопустимый формат файла.";
}
...
function form_useful_file_save($replace)
{
	global $path, $conn_id, $sql_pref;
	if ($replace!=="")
	{
	$sql_query="SELECT file_name FROM ".$sql_pref."_useful WHERE id='$replace'";
	$sql_res=mysql_query($sql_query, $conn_id);
	$name_to_del=mysql_fetch_row($sql_res);
	$dest_to_del=$path."files/useful/catalog/".$name_to_del[0];
	if (strlen($name_to_del[0])>0) del_file($dest_to_del);
	$sql_query="UPDATE ".$sql_pref."_useful SET file_name='' WHERE id='$replace'";
	$sql_res=mysql_query($sql_query, $conn_id);
	}
...
function del_useful($id)
{
	global $path, $conn_id, $sql_pref;
	$sql_query="SELECT file_name FROM ".$sql_pref."_useful WHERE id='$id'";
	$sql_res=mysql_query($sql_query, $conn_id);
	$name_to_del=mysql_fetch_row($sql_res);
	$dest_to_del=$path."files/useful/catalog/".$name_to_del[0];
	if (strlen($name_to_del[0])>0) del_file($dest_to_del);
	del_file($path."/files/useful/catalog/imgs/".$id.".jpg");
	del_file($path."/files/useful/catalog/imgs/".$id.".gif");
	del_record('useful', $id, 'No', -1);
}
?>
